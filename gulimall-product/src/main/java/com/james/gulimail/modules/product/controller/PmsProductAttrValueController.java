...
import com.james.gulimail.modules.product.excel.PmsProductAttrValueExcel;
...
public class PmsProductAttrValueController {
    ...
    //@RequiresPermissions("product:pmsproductattrvalue:page")
    public Result<PageData<PmsProductAttrValueDTO>> page(@Parameter(hidden = true) @RequestParam Map<String, Object> params){
        PageData<PmsProductAttrValueDTO> page = pmsProductAttrValueService.page(params);

        return new Result<PageData<PmsProductAttrValueDTO>>().ok(page);
    }

    @GetMapping("{id}")
    @Operation(summary = "信息")
    //@RequiresPermissions("product:pmsproductattrvalue:info")
    public Result<PmsProductAttrValueDTO> get(@PathVariable("id") Long id){
        PmsProductAttrValueDTO data = pmsProductAttrValueService.get(id);

        return new Result<PmsProductAttrValueDTO>().ok(data);
    }
    ...
    public Result save(@RequestBody PmsProductAttrValueDTO dto){
        //效验数据
        ValidatorUtils.validateEntity(dto, AddGroup.class, DefaultGroup.class);

        pmsProductAttrValueService.save(dto);
    ...
    public Result update(@RequestBody PmsProductAttrValueDTO dto){
        //效验数据
        ValidatorUtils.validateEntity(dto, UpdateGroup.class, DefaultGroup.class);

        pmsProductAttrValueService.update(dto);
    ...
    public Result delete(@RequestBody Long[] ids){
        ...
        pmsProductAttrValueService.delete(ids);
    ...
    @LogOperation("导出")
    //@RequiresPermissions("product:pmsproductattrvalue:export")
    public void export(@Parameter(hidden = true) @RequestParam Map<String, Object> params, HttpServletResponse response) throws Exception {
        List<PmsProductAttrValueDTO> list = pmsProductAttrValueService.list(params);

        ExcelUtils.exportExcelToTarget(response, null, "spu属性值", list, PmsProductAttrValueExcel.class);
    }
