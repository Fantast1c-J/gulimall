...
import com.james.gulimail.modules.product.excel.PmsBrandExcel;
...
public class PmsBrandController {
    ...
    //@RequiresPermissions("product:pmsbrand:page")
    public Result<PageData<PmsBrandDTO>> page(@Parameter(hidden = true) @RequestParam Map<String, Object> params){
        PageData<PmsBrandDTO> page = pmsBrandService.page(params);

        return new Result<PageData<PmsBrandDTO>>().ok(page);
    }

    @GetMapping("{id}")
    @Operation(summary = "信息")
    //@RequiresPermissions("product:pmsbrand:info")
    public Result<PmsBrandDTO> get(@PathVariable("id") Long id){
        PmsBrandDTO data = pmsBrandService.get(id);

        return new Result<PmsBrandDTO>().ok(data);
    }
    ...
    public Result save(@RequestBody PmsBrandDTO dto){
        //效验数据
        ValidatorUtils.validateEntity(dto, AddGroup.class, DefaultGroup.class);

        pmsBrandService.save(dto);
    ...
    public Result update(@RequestBody PmsBrandDTO dto){
        //效验数据
        ValidatorUtils.validateEntity(dto, UpdateGroup.class, DefaultGroup.class);

        pmsBrandService.update(dto);
    ...
    @LogOperation("导出")
    //@RequiresPermissions("product:pmsbrand:export")
    public void export(@Parameter(hidden = true) @RequestParam Map<String, Object> params, HttpServletResponse response) throws Exception {
        List<PmsBrandDTO> list = pmsBrandService.list(params);

        ExcelUtils.exportExcelToTarget(response, null, "品牌", list, PmsBrandExcel.class);
    }
